...
var _modifiers = __webpack_require__(303);
...
var store = (0, _createStore.createStore)(_modifiers2.default, {
  text: 'Initial'
});
...
store.subscribe('test', function (state) {
  return document.getElementById('container').innerHTML = state.text;
});
...
var createStore = exports.createStore = function createStore(modifiers, initialState) {
  ...
  var contactSubscriber = function contactSubscriber(state, action) {

    var subscribers = _subscribers.filter(function (subscriber) {
  ...
  var dispatch = function dispatch(task) {

    if (typeof task.action === 'undefined') {
      throw new Error('Dispatched task is expected to have an action property.');
    }

    var modifiers = _modifiers.filter(function (modifier) {
      return modifier.action === task.action;
    });

    updateState(modifiers.reduce(function (state, modifier) {
      return modifier.callback(state, task);
    }, _state), task.action);
  };

  return { subscribe: subscribe, dispatch: dispatch, getState: getState };
};
...
var _createModifier = __webpack_require__(304);
...
var modifiers = [{
  action: 'test',
  callback: function callback(state, task) {
    return task.modify ? { text: 'Final' } : state;
  }
}];
...
modifiers.push((0, _createModifier.createModifier)('test2', function (state, task) {
  return console.log(state, task);
}));
...
var createModifier = exports.createModifier = function createModifier(action, callback) {
